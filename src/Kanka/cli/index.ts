...
command
    ...
    .action(async () => {
        ...
        if (!process.env.KANKA_API_KEY) {
            ...
            console.error(
                "Please set this variable in your .env file or run 'bun run kanka config'"
            );
            process.exit(1);
        }
        ...
        try {
            ...
            const clientLayer = pipe(ClientServicesLive, Layer.provide(baseLayer));

            // 5. Create the campaigns API layer that depends on the client layer
            const campaignsLayer = pipe(CampaignsApiLive, Layer.provide(clientLayer));

            // 6. Create the entities API layer that depends on the client layer
            const entitiesLayer = pipe(EntitiesApiLive, Layer.provide(clientLayer));
            ...
            const program = async () => {
                try {
                    ...
                    const campaign = await Effect.runPromise(
                        pipe(selectCampaign(), Effect.provide(apiLayer)) as Effect.Effect<
                            Campaign | null,
                            never,
                            never
                        >
