...
const formatEntityDetails = (entity: Entity): string => {
    const details = [`ID: ${entity.id}`, `Name: ${entity.name}`, `Type: ${entity.type}`];

    if (entity.entry) {
        // Limit description length for display
        const truncatedEntry =
            entity.entry.length > 500 ? entity.entry.substring(0, 500) + "..." : entity.entry;
    ...
    return Effect.gen(function* (_) {
        ...
        while (!exitDetails) {
            ...
            if (entity.image) {
                ...
                try {
                    const renderedImage = yield* Effect.promise(() =>
                        fetchAndRenderImage(entity.image)
                    );
            ...
            switch (action) {
                ...
                case "edit": {
                    ...
                    const refreshedEntity = yield* fetchEntityById(
                        campaign,
                        entity.id as number,
                        entity.type
                    );
...
const displayEntitiesByType = (
    ...
    return Effect.gen(function* (_) {
        ...
        try {
            ...
            const entitiesArray = Array.isArray(response.data)
                ? response.data
                    ...
                    inquirer.prompt([
                        {
                            type: "input",
                            name: "continue",
                            message: "Press Enter to return to entity types",
                        },
                    ])
                );
                return;
 ...
 */
export const displayEntityMenu = (campaign: Campaign): Effect.Effect<void, unknown, unknown> => {
    return Effect.gen(function* (_) {
        let exitMenu = false;
        while (!exitMenu) {
            // Clear screen for better UX
            ...
            const entityTypes: Array<{
                name: string;
                value: EntityType | "all" | "back" | "create";
            }> = [
            ...
            if (selection === "back") {
                ...
                yield* Effect.catchAll(displayAllEntities(campaign), (error) =>
                    Effect.logInfo(`Error displaying all entities: ${String(error)}`)
                );
            } else if (selection === "create") {
                yield* Effect.catchAll(selectEntityTypeAndCreate(campaign), (error) =>
                    Effect.logInfo(`Error creating entity: ${String(error)}`)
                );
            } else {
