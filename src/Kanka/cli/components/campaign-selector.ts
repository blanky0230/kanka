 ...
 */
const displayCampaignSelector = (campaigns: Campaign[]): Effect.Effect<Campaign | null, never, never> => {
    return Effect.async<Campaign | null, never, never>((resume) => {
        ...
        inquirer
            ...
            .catch((error) => {
                Effect.logError(`Error selecting campaign: ${error.message}`).pipe(
                    Effect.runSync
                );
                resume(Effect.succeed(null));
            });

        // We don't need to return anything here - Effect.async completes via resume
        return;
    });
};
 ...
 */
export const selectCampaign = (): Effect.Effect<Campaign | null, string | ParseError, any> => {
