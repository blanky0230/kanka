...
import { CampaignSchema } from "../api/campaigns/types.js";
...
const UserIdSchema = Schema.Positive.pipe(Schema.fromBrand(PositiveInteger), Schema.fromBrand(UserIdBrand));
const EntityIdBrand = Brand.nominal<PositiveInteger>()
export const EntityIdSchema = Schema.Positive.pipe(Schema.fromBrand(PositiveInteger), Schema.fromBrand(EntityIdBrand));
