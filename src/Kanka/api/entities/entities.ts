 ...
 */
import { Context, Effect, Layer, RequestResolver, Schema, pipe } from "effect";
 ...
 */
export class EntityListByTypeRequest extends Schema.TaggedRequest<EntityListByTypeRequest>()(
    "EntityListByType",
    {
        failure: Schema.String,
        success: PaginatedResponseSchema(EntitySchema),
 ...
 */
export class EntityCreateRequest extends Schema.TaggedRequest<EntityCreateRequest>()(
    "EntityCreate",
    {
        failure: Schema.String,
        success: Schema.Struct({ data: EntitySchema }),
 ...
 */
export class EntityUpdateRequest extends Schema.TaggedRequest<EntityUpdateRequest>()(
    "EntityUpdate",
    {
        failure: Schema.String,
        success: Schema.Struct({ data: EntitySchema }),
 ...
 */
export class EntityDeleteRequest extends Schema.TaggedRequest<EntityDeleteRequest>()(
    "EntityDelete",
    {
        failure: Schema.String,
...
export class EntitiesApiService extends Context.Tag("EntitiesAPI")<
    EntitiesApiService,
    {
        ...
        listByType: (
            req: EntityListByTypeRequest
        ) => Effect.Effect<EntityList, string | ParseError>;
    ...
    Effect.gen(function* () {
        ...
        return {
            list: (req: EntityListRequest) => {
                ...
                return pipe(task, Effect.annotateLogs("entities", "list"));
            },
            getById: (req: EntityById) => {
                ...
                return pipe(task, Effect.annotateLogs("entities", "getById"));
            },
            listByType: (req: EntityListByTypeRequest) => {
                ...
                return pipe(task, Effect.annotateLogs("entities", "listByType"));
            },
            create: (req: EntityCreateRequest) => {
                const task = Effect.request(
                    ...
                    RequestResolver.fromEffect((_) =>
                        client.post(
                            `campaigns/${req.campaignId}/${req.entityType}s`,
                            req.data,
                            undefined
                        )
                    )
                ).pipe(Effect.tap(Effect.log));
                return pipe(task, Effect.annotateLogs("entities", "create"));
            },
            update: (req: EntityUpdateRequest) => {
                const task = Effect.request(
                    ...
                    RequestResolver.fromEffect((_) =>
                        client.patch(
                            `campaigns/${req.campaignId}/${req.entityType}s/${req.id}`,
                            req.data,
                            undefined
                        )
                    )
                ).pipe(Effect.tap(Effect.log));
                return pipe(task, Effect.annotateLogs("entities", "update"));
            },
            delete: (req: EntityDeleteRequest) => {
                ...
                return pipe(task, Effect.annotateLogs("entities", "delete"));
            },
        };
    })
);
