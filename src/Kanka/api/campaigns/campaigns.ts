    ...
    Effect.gen(function* () {
        ...
        return {
            list: (req: CampaignListRequest) => {
                ...
                return pipe(task, Effect.annotateLogs("campaigns", "list"));
            },
            getById: (req: CampaignById) => {
                ...
                return pipe(task, Effect.annotateLogs("campaigns", "getById"));
            },
            update: (req: CampaignMutation) => {
                const task = Effect.request(
                    req,
                    RequestResolver.fromEffect((_) =>
                        client.patch(`campaigns/${req.data.id}`, req.data, undefined)
                    )
                ).pipe(Effect.tap(Effect.log));
                return pipe(task, Effect.annotateLogs("campaigns", "update"));
            },
        };
    })
);
